...
class InsertAfterTest extends AbstractFunctionalTreeTestCase
{
    ...
    public function insert_after(): void
    {
        ...
        $this->assertSame(1, $node31->levelValue());
        $this->assertEquals(6, $node31->leftValue());
        $this->assertEquals(7, $node31->rightValue());

        $this->assertTrue($node41->isEqualTo($node31->prev()->first()));
        $this->assertTrue($node21->isEqualTo($node41->prev()->first()));

        $this->assertNull($node21->prev()->first());
        $this->assertNull($node31->next()->first());

        $this->assertTrue($node31->isEqualTo($node41->next()->first()));
        $this->assertTrue($node41->isEqualTo($node21->next()->first()));
    }
}
