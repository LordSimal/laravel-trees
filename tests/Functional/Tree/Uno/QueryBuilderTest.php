<?php

declare(strict_types=1);

namespace LordSimal\LaravelTrees\Tests\Functional\Tree\Uno;

use LordSimal\LaravelTrees\Tests\Functional\AbstractFunctionalTreeTestCase;
use LordSimal\LaravelTrees\Tests\Models\Category;
use PHPUnit\Framework\Attributes\Test;

class QueryBuilderTest extends AbstractFunctionalTreeTestCase
{
    /**
     * @return class-string<Category>
     */
    protected function modelClass(): string
    {
        return Category::class;
    }

    #[Test]
    public function root(): void
    {
        /** @var Category $modelRoot */
        $modelRoot = $this->model(['title' => 'root node']);
        $modelRoot->makeRoot()->save();

        /** @var Category $node21 */
        $node21 = $this->model(['title' => 'child 2.1']);
        $node21->appendTo($modelRoot)->save();

        /** @var Category $node31 */
        $node31 = $this->model(['title' => 'child 3.1']);
        $node31->appendTo($node21)->save();

        $modelRoot->refresh();

        $this->assertTrue($modelRoot->isEqualTo($node31->root()->first()));
        $this->assertCount(1, $node31->root()->get());
        $this->assertTrue($modelRoot->isEqualTo($node31->root()->get()->first()));
        $this->assertTrue($modelRoot->isEqualTo($node31->getRoot()));

        $this->assertTrue($modelRoot->isEqualTo(Category::root()->first()));
        $this->assertCount(1, Category::root()->get());
        $this->assertTrue($modelRoot->isEqualTo(Category::root()->get()->first()));
    }

    #[Test]
    public function not_root(): void
    {
        /** @var Category $modelRoot */
        $modelRoot = $this->model(['title' => 'root node']);
        $modelRoot->makeRoot()->save();

        /** @var Category $node21 */
        $node21 = $this->model(['title' => 'child 2.1']);
        $node21->appendTo($modelRoot)->save();

        /** @var Category $node31 */
        $node31 = $this->model(['title' => 'child 3.1']);
        $node31->appendTo($node21)->save();

        $list = Category::query()->notRoot()->get();

        $this->assertCount(2, $list);
    }
}
